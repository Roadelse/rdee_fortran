...
Subroutine test_rdDateTime()
    ...
    dtm1 = rdDateTime(y=2018, m=1, d=1, hh=22)
    print *, dtm1%toString()
    call assert(dtm1%toString() .eq. '2018010122', 'Error in rdDateTime%toString')
    print *, dtm1%toString('%Y/%m/%d %H')
    call assert(dtm1%toString('%Y/%m/%d %H') .eq. '2018/01/01 22', 'Error in rdDateTime%toString, with format')

    dtm1 = rdDateTime('20231007 1513')
    ! call dtm1%print
    call assert(dtm1%toString('%y/%m/%d %H') .eq. '23/10/07 15', 'Error in rdDateTime%fromString, from rdee_time, test 1')
    call dtm1%fromString('20231007 1513', format='%Y%m%d %H%M')
    ! call dtm1%print
    call assert(dtm1%toString('%y/%m/%d %H') .eq. '23/10/07 15', 'Error in rdDateTime%fromString, from rdee_time, test 2')
